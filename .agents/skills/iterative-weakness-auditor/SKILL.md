...
## Layer 0: Numerical Ground-Truth Benchmarking (Runs BEFORE all other layers)

This layer is mandatory and runs first. It catches implementation bugs that are invisible to algebraic analysis.

### 0.1 The Known-Limit Test
For every new numerical solver or integral:
- **Identify at least one exact closed-form limiting case** (EdS for collapse, power-law for variance, PS for MF, etc.)
- **Run the code at that limit** and compare to the analytic answer
- **Report the fractional error explicitly** — do not simply assert it "agrees"
- If the error exceeds 1%, characterize whether it is (a) a known systematic with documented source, or (b) a coding error. Log the answer in the issues_log regardless.

### 0.2 The Convergence Test
For any result that depends on a numerical parameter (ODE tolerance, integration limit, threshold, grid size):
- Vary the parameter by a factor of 10 in both directions
- Confirm the result changes by less than the claimed precision
- If the result IS sensitive to the parameter, this is a **convergence failure** — log it as a new active frontier

### 0.3 The Normalization Audit
For any adopted fitting formula or multiplicity function:
- Integrate it over its full domain and print the result
- Compare to the theoretical expectation (unity for normalized probability functions, ρ_m/M for mass functions)
- If the normalization is not unity, explicitly categorize: (a) calibration convention (cancels in ratios), (b) physical defect (kills absolute predictions)

### 0.4 The Docstring Honesty Check
Read the script's docstring before accepting any result as resolved. Flag any use of "exact", "precise", "rigorous", "zero free parameters" that is not quantitatively supported by the Layer 0 benchmark output.
