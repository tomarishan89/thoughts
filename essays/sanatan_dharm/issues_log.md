...
- [X] **ISSUE-2.2: First-Principles Viscoelastic Memory Kernel Derivation.** Derived $G(t-\tau) = G_0 e^{-(t-\tau)/\tau_{\text{relax}}} \Theta(t-\tau)$ directly from the Maxwell linear viscoelastic momentum balance in Section 1.2.
...
- [ ] **ISSUE-4.3 (Active Vulnerability #4): Inter-Tier Coupling Operator Variational Derivation.** Formal variational derivation of $\mathcal{O}_{\text{coupling}}^{m \to n} \equiv \frac{\delta \Psi[\mathbb{S}]}{\delta E^j}$ from the collective state-trace functional.
