...
| [recombination_inflow_cmb.py](recombination_inflow_cmb.py) | ISSUE-4.92 / §6.9.7 | Boltzmann solver (CAMB) with dynamic ADAF mass inflow at recombination ( $\Omega_m(z_{\text{rec}}) = 0.3153$, $\Omega_c h^2 = 0.1208$ ) | Collapses RMS residual from 4.18% to **0.51%** (peak 1: 0.00%, $r_s = 147.00\text{ Mpc}$ ) | `numpy`, `scipy`, `matplotlib`, `camb` |
