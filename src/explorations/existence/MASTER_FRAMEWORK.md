...
The manifestation of the engine across physical, biological, cognitive, and societal domains is classified by two fundamental parameters:
- **Predictive Complexity ( $\chi^*$ ):** The temporal depth and computational resource allocated to anticipatory simulation.
- **Memory Ledger ( $\mathcal{F}_{\text{ledger}}$ ):** The physical substrate that records past operational cycles and constitutive Lie algebra generators.
...
- **Governing Law:** Evaluated strictly at $\chi^* \equiv 0$. The system possesses no internal counterfactual model; boundaries deform according to local hyperbolic equations of motion and Einstein field equations.
...
- **Governing Law:** $\chi^* > 0$. The entity actively allocates chemical exergy to maintain a non-zero structural yield margin against external mechanical shocks and osmotic pressures.
...
- **Governing Law:** $\chi^* \gg 0$. The state space expands into the imaginary sector: $\exists\, E_{\text{self}}^{\mathfrak{Im}} \in \Omega_{\mathfrak{Im}}$ that models the entity and its environment.
...
- **Governing Law:** $\chi^* \ggg 0$. Macro-syncytial envelopes enclosing millions of constituent cognitive nodes ( $\{E^j\} \in T_{\text{III}}$ ).
