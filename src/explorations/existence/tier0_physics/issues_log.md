...
- **Formal Resolution:**  
  As proven in [`tier0_physics_framework.md`](tier0_physics_framework.md) §2, P1–P4 are derived structural theorems:
...
$$\mathbf{A}_{\text{phase}} = \frac{1}{T} \int_0^T \left( \hat{\mathbf{r}}_{\text{ast}}(t) \otimes \hat{\mathbf{r}}_{\text{Jup}}(t) \right) dt$$
  ...
  For Hilda (3:2), the resonant libration of the conjunction longitude prevents close encounters, yielding a persistent anisotropic tensor eigenvalue structure ( $\lambda_{\text{max}}/\lambda_{\text{min}} > 4.0$ ). For Kirkwood (3:1), chaotic phase wandering washes out the directional correlation ( $\lambda_{\text{max}}/\lambda_{\text{min}} \to 1.0$ ). Numerically verified in [`../scripts/threebody_player_hierarchy.py`](../scripts/threebody_player_hierarchy.py). Closed in [`tier0_physics_framework.md`](tier0_physics_framework.md) §7.
